...
import { LIMITS, dateKey, formatShort, groupByDay } from "@/lib/records";
...
export default function RecordScreen({ dateParam }: { dateParam?: string }) {
  const { records, ready, add } = useRecords();
  ...
  const save = () => {
    ...
    setError(null);
    add({ recordedAt: recordedAt.toISOString(), temperature: t, systolic: s, diastolic: d });
    setTemp("");
    setSys("");
    setDia("");
  };
  ...
  return (
    <>
      <header className="px-1 pb-2">
        ...
        <p className="mt-1 text-sm text-sub">매일 같은 시간에 기록하면 추세가 더 정확해져요</p>
      </header>
      ...
      <section className="mt-2.5 flex flex-col gap-2.5">
        <h2 className="px-1 text-[15px] font-bold">{isToday ? "오늘 기록" : "이 날의 기록"}</h2>
        {dayRecords.length === 0 && (
        ...
        {dayRecords.map((r) => (
          <div
            key={r.id}
            className="flex items-center justify-between rounded-[18px] bg-white px-[18px] py-4 shadow-card"
          >
